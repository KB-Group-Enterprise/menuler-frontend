  ...
  >
    <div class="container-fluid w-full flex flex-wrap items-center justify-between px-6">
      ...
      <div class="flex items-center relative">
        ...
        <div class="dropdown relative">
          <span
            class="dropdown-toggle flex items-center hidden-arrow cursor-pointer"
            href="#"
          ...
          >
            <div>
                {{ auth.state.user?.restaurant.restaurantName }}
                <div class="flex justify-center items-center space-x-1 text-xs">
                    <div
                    :class="[auth.state.user?.restaurant.status === 'OPEN' ? 'bg-green-500' : 'bg-gray-400']"
                    class="w-3 h-3 rounded-full"
                    ></div>
                    <div>{{ auth.state.user?.restaurant.status === 'OPEN' ? 'เปิดให้บริการ' : 'ปิดให้บริการ' }}</div>
                </div>
            </div>
          </span>
          <ul
